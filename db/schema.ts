...
export const linkedAccounts = pgTable("linked_accounts", {
  ...
  uniqueIndex("linked_google_subject_idx").on(table.googleSubject),
...
export const chunks = pgTable("chunks", {
  ...
  uniqueIndex("chunks_file_part_idx").on(table.fileId, table.part),
  index("chunks_account_idx").on(table.accountId),
]);
...
export const userSettings = pgTable("user_settings", {
  ...
  preferences: jsonb("preferences").$type<MeshlyPreferences>().notNull().default(defaultPreferences),
  updatedAt: timestamp("updated_at", { withTimezone: true }).defaultNow().notNull(),
});
...
export const shares = pgTable("shares", {
  ...
  uniqueIndex("shares_token_idx").on(table.tokenHash),
  index("shares_user_idx").on(table.userId, table.createdAt),
  index("shares_file_idx").on(table.fileId),
]);

export const syncState = pgTable("sync_state", {
  accountId: text("account_id").primaryKey().references(() => linkedAccounts.id, { onDelete: "cascade" }),
  changePageToken: text("change_page_token"),
  lastQuotaRefresh: timestamp("last_quota_refresh", { withTimezone: true }),
  lastRecoverySnapshot: timestamp("last_recovery_snapshot", { withTimezone: true }),
