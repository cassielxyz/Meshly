...
export function DriveView({ parentId = null, scope = "folder", query = "" }: { parentId?: string | null; scope?: Scope; query?: string }) {
  ...
  useEffect(() => {
    ...
    return () => window.removeEventListener("meshly:refresh", refresh);
  }, [load]);
  ...
  async function share(item: Item) {
    ...
    const response = await fetch("/api/shares", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ fileId: item.id, password: password || null, expiresHours: 168 }),
    });
  ...
  async function startMove(item: Item) {
    ...
    setFolders(data.items.filter((candidate) => candidate.kind === "folder" && candidate.id !== item.id));
    setMoveItem(item);
  }
  ...
  function itemMenu(item: Item) {
    return (
      <div className="absolute right-0 top-10 z-30 w-48 rounded-xl border border-[var(--border)] bg-[var(--background)] p-1.5 text-sm shadow-xl">
        {item.kind === "file" && <>
          <button onClick={() => window.open(`/api/files/${item.id}/preview`, "_blank")} className="menu-item"><Eye size={15}/>Preview</button>
          <a href={`/api/files/${item.id}/download`} className="menu-item"><Download size={15}/>Download</a>
          <button onClick={() => void share(item)} className="menu-item"><Share2 size={15}/>Share</button>
        </>}
        {!item.trashedAt && <>
          <button onClick={() => void rename(item)} className="menu-item">Rename</button>
          <button onClick={() => void startMove(item)} className="menu-item"><Move size={15}/>Move</button>
          <button onClick={() => void patch(item, { op: "star", starred: !item.starred })} className="menu-item"><Star size={15}/>{item.starred ? "Unstar" : "Star"}</button>
          <button onClick={() => void patch(item, { op: "trash" })} className="menu-item text-[var(--red)]"><Trash2 size={15}/>Move to trash</button>
        </>}
        {item.trashedAt && <>
          <button onClick={() => void patch(item, { op: "restore" })} className="menu-item"><RotateCcw size={15}/>Restore</button>
          <button onClick={() => void permanent(item)} className="menu-item text-[var(--red)]"><Trash2 size={15}/>Delete forever</button>
        </>}
  ...
  return <div className="min-h-[calc(100vh-64px)] p-4 sm:p-6">
    <div className="flex flex-wrap items-start justify-between gap-3">
      <div>
        ...
        {scope === "folder" && !query && <div className="mt-2 flex flex-wrap items-center gap-1 text-xs text-[var(--muted)]">
          {breadcrumbs.map((crumb, index) => <span className="flex items-center gap-1" key={crumb.id ?? "root"}>{index > 0 && <ChevronRight size={13}/>}<Link className="rounded px-1 py-0.5 hover:bg-[var(--background)]" href={crumb.id ? `/drive/${crumb.id}` : "/drive"}>{crumb.name}</Link></span>)}
        </div>}
      </div>
      <div className="flex items-center gap-2">
        ...
        <div className="flex items-center rounded-full border border-[var(--border)] bg-[var(--background)] p-1">
          <button aria-label="List view" onClick={() => setGrid(false)} className={`grid h-8 w-10 place-items-center rounded-full ${!grid ? "bg-[var(--surface-strong)]" : ""}`}><List size={17}/></button>
          <button aria-label="Grid view" onClick={() => setGrid(true)} className={`grid h-8 w-10 place-items-center rounded-full ${grid ? "bg-[var(--surface-strong)]" : ""}`}><Grid2X2 size={17}/></button>
        </div>
      </div>
    </div>
    ...
    {details && <div className="fixed inset-y-0 right-0 z-[75] w-full max-w-md overflow-auto border-l border-[var(--border)] bg-[var(--background)] p-5 shadow-2xl"><div className="flex items-center justify-between"><h2 className="font-semibold">File details</h2><button onClick={() => setDetails(null)}><X size={20}/></button></div><h3 className="mt-6 break-words text-lg font-medium">{details.item.name}</h3><dl className="mt-5 grid grid-cols-[120px_1fr] gap-y-3 text-sm"><dt className="text-[var(--muted)]">Type</dt><dd>{details.item.mimeType}</dd><dt className="text-[var(--muted)]">Size</dt><dd>{fmtSize(details.item.size)}</dd><dt className="text-[var(--muted)]">Status</dt><dd className="capitalize">{details.item.status}</dd><dt className="text-[var(--muted)]">Modified</dt><dd>{fmtDate(details.item.modifiedAt)}</dd><dt className="text-[var(--muted)]">SHA-256</dt><dd className="break-all font-mono text-xs">{details.item.sha256 ?? "—"}</dd></dl>{details.allocation.length > 0 && <><h4 className="mt-7 text-sm font-semibold">Physical allocation</h4><div className="mt-2 space-y-2">{details.allocation.map((allocation) => <div key={`${allocation.accountId}-${allocation.part}`} className="rounded-xl bg-[var(--surface)] p-3 text-xs"><div className="font-medium">Part {allocation.part + 1} · {fmtSize(allocation.size)}</div><div className="mt-1 text-[var(--muted)]">{allocation.accountEmail} · {allocation.status}</div></div>)}</div></>}</div>}
